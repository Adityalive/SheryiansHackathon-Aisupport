...
const STAGES = [
  {
    ...
    fields: [
      ...
      { label: "Website / Store URL", placeholder: "e.g., www.trendcart.com" },
      {
        ...
        placeholder: "e.g., We sell trendy fashion...",
        type: "textarea",
      },
      {
        label: "Social Media Handles",
        placeholder: "e.g., @trendcart on Instagram",
      },
    ],
  },
  {
    ...
    fields: [
      {
        label: "Return Policy",
        placeholder:
          "e.g., 7 days from delivery date, unused with original tags. Non-returnable: Innerwear.",
        type: "textarea",
      },
      {
        label: "Refund Policy",
        placeholder:
          "e.g., 5-7 business days to original payment method. Free return shipping.",
        type: "textarea",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      },
      {
        ...
        placeholder: "e.g., 2–3 business days (extra charge)",
      },
      {
        label: "Same-Day / Next-Day Delivery",
        placeholder: "e.g., Available in select cities",
      },
      {
        label: "Free Shipping Threshold",
        placeholder: "e.g., Orders above ₹999 get free shipping",
      },
      {
        label: "Shipping Charges",
        placeholder: "e.g., ₹49 flat fee for orders below ₹999",
      },
      {
        label: "International Shipping",
        placeholder: "e.g., Yes/No + countries + timeline",
      },
      {
        label: "Non-Serviceable Areas",
        placeholder: "e.g., List of pin codes or regions not covered",
      },
      {
        label: "Tracking Info",
        placeholder: "e.g., Tracking link is sent via SMS/email after dispatch",
      },
      {
    ...
    fields: [
      {
        ...
        placeholder: "e.g., Orders can be cancelled within 1 hour of placing",
        type: "textarea",
      },
      {
        label: "How to Cancel",
        placeholder: "e.g., Via dashboard or email to support",
        type: "textarea",
      },
      {
        label: "Order Modification Policy",
        placeholder: "e.g., Address can be changed before dispatch only",
        type: "textarea",
      },
      {
        label: "Out-of-Stock Handling",
        placeholder: "e.g., Full refund issued within 3 business days",
        type: "textarea",
      },
      {
        label: "Order Confirmation Method",
        placeholder: "e.g., Email + SMS confirmation sent immediately",
        type: "textarea",
      },
      {
        label: "Invoice / Bill Availability",
        placeholder: "e.g., Digital invoice sent via email",
        type: "textarea",
      },
      {
        label: "COD (Cash on Delivery) Availability",
        placeholder: "e.g., Yes/No + extra charges if any",
        type: "textarea",
      },
      {
        label: "Pre-Order Policy",
        placeholder: "e.g., Timeline and payment terms for pre-orders",
        type: "textarea",
      },
    ],
  },
  {
    ...
    fields: [
      ...
      {
        label: "Size Chart",
        placeholder: "e.g., Link or description of size guide",
      },
      {
        label: "Materials / Fabric Info",
      ...
      },
      {
        ...
        placeholder: "e.g., Name printing, engraving, etc.",
      },
      {
        label: "Product Warranty",
        placeholder: "e.g., 6 months warranty on electronics",
      },
      {
        label: "Authenticity Guarantee",
        placeholder: "e.g., All products are 100% original",
      },
      {
        label: "Gift Wrapping",
        placeholder: "e.g., Available / Not available + cost",
      },
    ],
  },
  {
    ...
    fields: [
      {
        ...
        placeholder: "e.g., UPI, Credit/Debit Card, Net Banking, Wallets, COD",
      },
      {
      ...
      },
      {
        label: "EMI Options",
        placeholder:
          "e.g., EMI available on orders above ₹3000 via select banks",
      },
      {
        label: "Payment Failure Handling",
        placeholder: "e.g., Amount auto-refunded in 3–5 business days",
      },
      {
        label: "Currency Accepted",
        placeholder: "e.g., INR / USD / Multi-currency",
      },
      {
        label: "Discount / Coupon Usage",
        placeholder: "e.g., One coupon per order, cannot be combined",
      },
      {
        label: "Loyalty / Points Program",
        placeholder: "e.g., Earn/redeem points info",
      },
      {
        label: "GST / Tax Info",
        placeholder: "e.g., 18% GST included in product price",
      },
    ],
  },
  {
    ...
    fields: [
      {
        label: "Account Creation Required?",
        placeholder:
          "e.g., Yes (for order tracking) / Guest checkout available",
        type: "textarea",
      },
      {
        label: "Password Reset Process",
        placeholder: "e.g., Use 'Forgot Password' on login page",
        type: "textarea",
      },
      {
        label: "Data Privacy Policy",
        placeholder: "e.g., Link to privacy policy page",
        type: "textarea",
      },
      {
        label: "Account Deletion Process",
        placeholder: "e.g., Email support to request account deletion",
        type: "textarea",
      },
      {
        label: "Newsletter Unsubscribe",
        placeholder: "e.g., Click unsubscribe link in any email",
        type: "textarea",
      },
      {
        label: "Data Sharing Policy",
        placeholder: "e.g., We do not sell your data to third parties",
        type: "textarea",
      },
    ],
  },
  {
    ...
    fields: [
      {
        label: "Current Active Offers",
        placeholder: "e.g., 20% off sitewide till May 10th",
      },
      {
        label: "Sale Events",
        placeholder: "e.g., Annual sale every January & July",
      },
      {
        label: "Referral Program",
        placeholder: "e.g., Refer a friend, get ₹100 credit each",
      },
      {
        label: "Student / Special Discounts",
        placeholder: "e.g., Eligibility and process",
      },
      {
        label: "Coupon Stacking Policy",
        placeholder: "e.g., Only one coupon per order",
      },
...
const KnowledgeBaseTab = ({ tenantId }) => {
  ...
  const { kbItems, kbLoading, fetchKbItems, deleteKbItem } =
    useDashboardStore();
  ...
  useEffect(() => {
    if (activeStageData && activeStageData.fields && kbItems.length > 0) {
      const initialData = {};
      activeStageData.fields.forEach((field) => {
        const existing = kbItems.find(
          (item) => item.question === field.label || item.title === field.label,
        );
        if (existing) {
          initialData[field.label] = existing.answer || existing.content;
        }
      });
      setFormData((prev) => {
        const next = { ...prev };
  ...
  }, [activeStage, kbItems, activeStageData]);
    ...
    try {
      await Promise.all(
        fieldsToSave.map(async (field) => {
          // Delete existing item for this field if it exists
          const existing = kbItems.find(
            (item) =>
              item.question === field.label || item.title === field.label,
          );
          if (existing) {
            await kbService.deleteKnowledgeBaseItem(tenantId, existing._id);
          }

          return kbService.addKnowledgeBaseItem(tenantId, {
            type: "faq",
            title: stage.label,
        ...
        <div className="px-6 py-4 border-b border-[#e1e3e4] flex items-center justify-between bg-white flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-[#777586] mt-1">
              Fill in the details to train your AI support agent.
            </p>
          </div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files?.length)
                      alert(
                        "CSV Upload functionality to be implemented for " +
                          activeStageData.label,
                      );
                  }}
                />
          ...
          {activeStageData?.id === "saved_data" ? (
            <div className="space-y-4">
              ...
              {kbLoading ? (
                ...
                <div className="text-sm text-[#777586] text-center py-8">
                  No knowledge base items yet. Start filling out the stages!
                </div>
              ) : (
                <div className="space-y-2">
                  {kbItems.map((item) => (
                    <div
                      key={item._id}
                      className="flex items-start justify-between gap-3 p-3 border border-[#e1e3e4] rounded-md hover:bg-[#f8f9fa] transition-colors"
                    >
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-sm font-medium text-[#191c1d] truncate">
                            {item.title}
                          </span>
                          <span className="text-[10px] font-medium bg-[#eef2ff] text-[#4338ca] px-1.5 py-0.5 rounded uppercase flex-shrink-0">
                        ...
                        <p className="text-xs text-[#777586] truncate">
                          {item.answer || item.content?.substring(0, 100)}
                        </p>
                      </div>
                      <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              {activeStageData?.fields?.map((field) => (
                ...
                >
                  <label className="block text-xs font-medium text-[#464554] mb-1.5">
                    {field.label}
                  </label>
                  {field.type === "textarea" ? (
                    <textarea
