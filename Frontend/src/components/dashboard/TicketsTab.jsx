...
const TicketsTab = ({ tenantId }) => {
  const { tickets, ticketLoading, resolveTicket, deleteTicket } =
    useDashboardStore();

  if (ticketLoading) {
    return (
      <div className="text-sm text-[#777586] py-4">Loading tickets...</div>
    );
  }
    ...
    <div className="max-w-3xl">
      <div className="bg-white border border-[#e1e3e4] rounded-lg p-6">
        ...
        <p className="text-sm text-[#777586] mb-5">
          Escalated conversations from voice and chat.
        </p>
        ...
        {tickets.length === 0 ? (
          ...
          <div className="space-y-3">
            {tickets.map((ticket) => (
              <div
                key={ticket._id}
                className="border border-[#e1e3e4] rounded-md p-4"
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span
                      className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-full ${PRIORITY_STYLES[ticket.priority] || PRIORITY_STYLES.low}`}
                    >
                      {ticket.priority}
                    </span>
